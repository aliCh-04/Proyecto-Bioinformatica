...
vector<string> generador(unordered_map<string, vector<char>> grafo) {
	...
	for (auto& [nodo, aristas] : grafo_limpio) {
		if (entradas[nodo] == 0 || salidas[nodo] > 1 || entradas[nodo] > 1) { //Hay una bifurcaci�n
			while (!aristas.empty()) {
				string n_actual = nodo;	//El nodo con el que estamos trabajando
				string contig = n_actual;	//El contig que se est� formando
				...
				while (entradas[n_actual] == 1 && salidas[n_actual] == 1) { //Se sigue formando el contig hasta una nueva bifurcaci�n
